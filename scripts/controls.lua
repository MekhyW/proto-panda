...
function _M.onLoop(dt)
	...
	for i=DEVICE_0_BUTTON_LEFT, DEVICE_0_BUTTON_CONFIRM do
		...
		local color = 0
		if readButtonStatus(i) == BUTTON_PRESSED or readButtonStatus(i) == BUTTON_JUST_PRESSED then
			color = 1
			hasPressed = true
	...
	drawPanelLine(8, 7, 8 + 3.5*readAccelerometerX(0), 7, color565(255,0,0))
	drawPanelLine(8, 7, 8 , 7 + 3.5*readAccelerometerY(0), color565(0,255,0))
	local vz = readAccelerometerZ(0)
	drawPanelLine(8, 7, 8 + 3.5*vz , 7+ 3.5*vz, color565(0,0,255))
	drawPanelLine(8, 29, 7+4*readAccelerometerX(0), 29, color565(255,0,0))
	drawPanelLine(8, 30, 7+4*readAccelerometerY(0), 30, color565(0,255,0))
	drawPanelLine(8, 31, 7+4*readAccelerometerZ(0), 31, color565(0,0,255))
	...
	drawPanelLine(16+8, 7, 16+8 + 3.5*readAccelerometerX(1), 7, color565(255,0,0))
	drawPanelLine(16+8, 7, 16+8 , 7 + 3.5*readAccelerometerY(1), color565(0,255,0))
	local vz = readAccelerometerZ(1)
	drawPanelLine(16+8, 7, 16+8 + 3.5*vz , 7+ 3.5*vz, color565(0,0,255))
	drawPanelLine(16+8, 29, 16 + 7+4*readAccelerometerX(1), 29, color565(255,0,0))
	drawPanelLine(16+8, 30, 16 + 7+4*readAccelerometerY(1), 30, color565(0,255,0))
	drawPanelLine(16+8, 31, 16 + 7+4*readAccelerometerZ(1), 31, color565(0,0,255))
	...
	local gx = map(readGyroX(0), -12000, 12000, -7, 7)
	local gy = map(readGyroY(0), -12000, 12000, -7, 7)
	local gz = map(readGyroZ(0), -12000, 12000, -7, 7)
	...
	local gx = map(readGyroX(1), -12000, 12000, -7, 7)
	local gy = map(readGyroY(1), -12000, 12000, -7, 7)
	local gz = map(readGyroZ(1), -12000, 12000, -7, 7)
